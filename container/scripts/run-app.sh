...
java -XX:+UseNUMA -XX:$GARBAGE_COLLECTOR -XX:+UseCompactObjectHeaders -XX:MaxRAMPercentage=$HEAP_MEMORY_PERCENTAGE -Dserver.ssl.key-store-type=PKCS12 -Dserver.ssl.key-store=/home/app/backend.p12 -Dserver.ssl.key-alias=backend -Dserver.ssl.enabled=true -Dserver.ssl.key-store-password=$RANDOM_STR -Dserver.port=8443 -jar springwebdemo.jar  --spring.config.location=/home/app/config.yml,/home/app/oidc.yml,/home/app/database.yml